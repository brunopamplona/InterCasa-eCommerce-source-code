...
class ControllerPaymentGN5 extends Controller {
	...
	public function criar() {
		...
		$customer = [
			'name' => utf8_decode(preg_replace('/\s+/', ' ',trim($order_info['firstname']).' '.trim($order_info['lastname']))),
		...
		$logradouro = utf8_decode(trim(str_replace(',','',preg_replace('/\d+/','',$order_info['payment_address_1']))));
		$bairro = utf8_decode((empty($order_info['payment_address_2'])?'Bairro':$order_info['payment_address_2']));
		$cep = $order_info['payment_postcode'];
		$cidade = utf8_decode($order_info['payment_city']);
