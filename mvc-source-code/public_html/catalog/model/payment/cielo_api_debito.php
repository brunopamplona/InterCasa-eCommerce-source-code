<?php
class ModelPaymentCieloApiDebito extends Model {
    public function getMethod($address, $total) {
        ...
        if ($status) {
            if (strlen(trim($this->config->get('cielo_api_debito_imagem'))) > 0) {
                $title = '<img src="'.HTTP_SERVER.'image/'.$this->config->get('cielo_api_debito_imagem').'" alt="'.$this->config->get('cielo_api_debito_titulo').'" />';
            } else {
