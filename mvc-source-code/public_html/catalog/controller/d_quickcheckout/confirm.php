...
class ControllerDQuickcheckoutConfirm extends Controller {
	...
	public function index($config){
        ...
        $this->data['col'] = $config['account']['guest']['confirm']['column'];
        $this->data['row'] = $config['account']['guest']['confirm']['row'];
   ...
   return  $this->response->setOutput($this->render());		
	...
	public function updateField(){
        ...
        $this->model_module_d_quickcheckout->updateStatistic($statistic);
			$this->model_module_d_quickcheckout->endDebug('Update:: Field');
    ...
    public function update(){
        ...
        if($this->customer->isLogged()){
            ...
            if($this->session->data['account'] == 'register'){
                ...
                $this->log->write($this->session->data['payment_address']);
        ...
        $this->updateOrder();
        $json['order_id'] = $this->session->data['order_id'] = $this->output;
        ...
        $this->model_module_d_quickcheckout->updateStatistic($statistic);
		$this->model_module_d_quickcheckout->endDebug('Update:: Confirm');
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json));
    }
    ...
    public function updateOrder(){
       ...
       // $order_data['payment_custom_field'] = (isset($this->session->data['payment_address']['custom_field']['address']) ? $this->session->data['payment_address']['custom_field']['address'] : array());
        ...
        $this->load->model('d_quickcheckout/order');
		$this->model_module_d_quickcheckout->endDebug('Update:: order '.$this->session->data['order_id'] );
 
		$this->output =  $this->model_d_quickcheckout_order->updateOrder($this->session->data['order_id'], $order_data);
    }
}