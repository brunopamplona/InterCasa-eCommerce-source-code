<?php
	$logFile = DIR_APPLICATION . "controller/payment/smcielopageloja_lib/logs/log.log";
	...
	function VerificaErro($vmPost, $vmResposta)
	{
		...
		catch  (Exception $ex)
		{
			...
			echo '<span style="font-weight:bold;">Detalhes do erroooo:</span>' . '<br />';
		...
		{
			$error_msg = "<b>Codigo do erro: </b>" . $objResposta->codigo . "<br />";
			//$error_msg .= "Mensagem: " . utf8_decode($objResposta->mensagem) . "\n";
			$error_msg .= "<b>Mensagem: </b>" . ($objResposta->mensagem) . "\n";
			// Gera página HTML
			//echo '<html><head><title>Erro na transação</title></head><body>';
			//echo '<span style="color:red;, font-weight:bold;">Ocorreu um erro em sua transação!</span>' . '<br />';
