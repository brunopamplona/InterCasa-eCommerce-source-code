	...
	protected function index($setting)
	{
		...
		foreach ($products as $product_id) {
			...
			if ($product_info) {
				...
				if ($product_info['image']) {
					$image1 = $this->model_tool_image->resize(
						$product_info['image'],
				...
				if (!empty($getImages)):
					$im       = reset($getImages);
					$imageAlt = $this->model_tool_image->resize($im['image'], $setting['image_width'], $setting['image_height']);
				else:
					$imageAlt = FALSE;
				endif;
				...
				$this->data['products'][] = array(
					...
					'manufacturers'     => $this->data['manufacturers'] = $this->url->link(
