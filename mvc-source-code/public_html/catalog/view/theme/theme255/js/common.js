$(document).ready(function() {
	...
	if ($('.container').width() < 980) {
			$('#cart .heading span.span_link').live("click", function(){
			...
			$('#cart > .heading span.span_link').live('mouseover', function() {
			$('#cart').addClass('active');
	...
	$('#menu ul > li > a + div').each(function(index, element) {
		...
		if ($.browser.msie && ($.browser.version == 7 || $.browser.version == 6)) {
			...
			$(element).css('width', (columns * 163) + 'px');
...
$(document).ready(function(){
    var eCep = cep = $("input[name='postcode']");
    eCep.live('keyup', function (event) {
        $this = $(this);
        if ($this.val().length == 8) {
            $("#autopreencher").trigger('click');
        }
    });
});