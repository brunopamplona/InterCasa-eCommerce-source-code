<?php
	...
	if ($order_id->row > 0):
		...
		if ($apiNip == $token):
			...
			elseif ($status == 'Aprovado - Separando Produtos'):
			...
			if ($notify == 1):
				...
				//Número do pedido
				$mensagem = $languageMail->get('text_update_order') . $order_id->row['order_id'] . '<br/><br/>';
				//Data de criação do pedido
				$mensagem .= $languageMail->get('text_update_date_added') . ' ' . date('d/m/Y h:m:s') . '<br/><br/>';
				//Status do pedido
				$mensagem .= $languageMail->get('text_update_order_status') . $status . '<br/><br/>';
				...
				$mensagem .= '<a href="'. HTTP_SERVER . 'index.php?route=account/order/info&order_id='. $order_id->row['order_id'] . '">'. HTTP_SERVER . 'index.php?route=account/order/info&order_id='. $order_id->row['order_id'] . '</a><br/><br/>';
...
$db->query("INSERT INTO `". DB_PREFIX ."order_history` (`order_id`, `order_status_id`, `notify`, `date_added`) VALUES ('". $order_id->row['order_id'] ."', '". $order_status_id ."', '".$notify."', NOW())");
