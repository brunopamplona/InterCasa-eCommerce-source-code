...
function sql_getUser($user, $password, $version) {
	...
	switch ($version) {
		...
		case "1.5.3":
			...
			break;
		default:
			$query = $db->query("SELECT * FROM `" . DB_PREFIX . "user` WHERE username = '" . mysql_real_escape_string($user) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . mysql_real_escape_string($password) . "'))))) OR password = '" . mysql_real_escape_string(md5($password)) . "') AND status = '1'");
			break;
	}
	return $query->rows;
}
