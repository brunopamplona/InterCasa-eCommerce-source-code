	...
	<div class="box">
		...
		<div class="content" style="overflow: hidden">
			<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
				...
				<input type="hidden" id="store_url" name="store_url" value="<?php echo $store_url; ?>" />
				<div id="tab-omframework">
					...
					<div id="tab-omframework-general">
						<table class="form">
							...
							<tr>
								<td valign=top><?php echo $entry_mobile_ic; ?></td>
								<td>
									<input type="checkbox" name="config_mobile_ic" value="1" <?php if ($config_mobile_ic){echo 'checked';}; ?> size="3" />
								</td>
							</tr>
							<tr>
								<td valign=top><?php echo $entry_mobile_disable_site_for_tablets; ?></td>
								<td>
			            ...
			            <table class="form">
							...
							<tr>
								...
								<td style="border-top: 1px dotted #CCCCCC;">
									<a class="button button-gen"><?php echo $button_generate;?></a>
									<a class="button btn-gen" href="" target="_blank" style="display: none"><?php echo $button_generate;?></a><?php echo " $text_gen_instruction"; ?>
								</td>
							</tr>
						</table>
					</div>
									...
									<table class="list">
										<thead>
											<tr>
												...
												<td class="center"><?php echo $text_smartphones; ?></td>
												<td class="center"><?php echo $text_tablets; ?></td>
											</tr>
										</thead>
										<?php foreach ($modules as $module_sysname => $module) { ?>
										<tbody>
											<tr>
												<td valign=top><?php echo $module['name']; ?></td>
												<td class="center">
													<input type="checkbox" name="config_modules_<?php echo $module_sysname; ?>_smartphones" value="1" size="3" />
												</td>
												<td class="center">
										            <input type="checkbox" name="config_modules_<?php echo $module_sysname; ?>_tablets" value="1" size="3" />
								            	</td>
											</tr>
										</tbody>
										<?php } ?>
...
function populate(json) {
	$('input[type=\'checkbox\']').not('[name="patches[]"], [name="config_mobile_disable_omf"]').removeAttr('checked');
	...
	$('#screenshot').load('index.php?route=module/omfa/getScreenShot&token=<?php echo $token; ?>&store=' + $('#store_id').val());
...
function jsonify(formSet, mobile_logo) {
	...
	for (var i in a) {
	if (a[i].name == 'patches[]' || a[i].name == 'config_mobile_disable_omf') continue; // skip patches
...
//console.log("loadStoreSettings store_id: "+ store_id +"\n store_name: "+ store_name);
	$.ajax({
		...
		success: function(json) {
			...
			if (json['store_id'] != 0) {
				$(".button-gen").hide();
				$(".btn-gen").attr("href", json['store_url'] + 'index.php?view=desktop');
				$(".btn-gen").show();
			} else {
				$(".btn-gen").hide();
				$(".button-gen").show();
...
function generateNewImage(e) {
	...
	if (location.protocol === "https:") {
		var catalog = "<?php echo $https; ?>";
		//console.log(catalog);
	} else {
		var catalog = "<?php echo HTTP_CATALOG; ?>";
	}
	var homePage = window.open(catalog + "index.php?view=desktop");
	window.focus();
	if (homePage) {
				...
				if (!homePage.closed) {
					...
					html2canvas(homePage.document.body, {
		    			onrendered: function(canvas) {
		    				$.ajax({
		    					type: "post",
		    					url: "index.php?route=module/omfa/saveScreenShot&store=" + $('#store_id').val() + "&token=<?php echo $token; ?>",
...
$('#tab-omframework-adv-customization').on('click', '.btn-gen', function() {
	$('.error, .success').remove();
	$('.box').before('<div id="warning" class="attention"><img src="view/image/loading.gif" alt="" /> <?php echo "$text_generate"; ?></div>');
	window.location.hash = '';
	window.location.hash = '#warning';
	...
	setTimeout(function () {
		$('#screenshot').load('index.php?route=module/omfa/getScreenShot&token=<?php echo $token; ?>&store=' + $('#store_id').val());
		$('.attention').remove();
		$('.box').before('<div class="success" style="display: none;"><?php echo $success_gen; ?></div>');
		$('.success').fadeIn('slow');
		$('.error').remove();
	}, 10000);
...
$('#tab-omframework-adv-customization').on('click', '.button-gen', generateNewImage);
