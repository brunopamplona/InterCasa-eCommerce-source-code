...
class ModelModuleDQuickcheckout extends Model {
    ...
    public function escape($data){
    	return htmlentities(stripslashes(utf8_encode($data)), ENT_QUOTES);
    }
   ...
   public function uninstallDatabase(){
    ...
    public function setSetting($setting_name, $setting_value, $store_id = 0){
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "dqc_setting` 
			SET store_id = '" . (int)$store_id . "', 
				...
				`value` = '" . $this->db->escape(serialize($setting_value)) . "'");
    ...
    public function getStatistics($setting_id){
    	...
    	foreach($query->rows as $key => $data){
    		$query->rows[$key]['data'] = $this->rateStatistic(unserialize($data['data']));
...
//		return $custom_fields;
	...
	*/
	public function getMboothInfo($mbooth_xml){
		if(file_exists(DIR_APPLICATION . 'mbooth/xml/'. $mbooth_xml)){
	...
	*/
	...
	public function installDependencies($mbooth){
		...
		foreach($this->getDependencies($mbooth) as $extension){
			if(isset($extension['codename'])){
				...
				if(file_exists(DIR_APPLICATION . 'mbooth/xml/'.$mbooth)){
	...
	public function download_extension($codename, $version, $filename  = false ) {
		...
		curl_setopt($ch, CURLOPT_URL, 'http://opencart.dreamvention.com/api/1/extension/download/?codename=quickcheckput&opencart_version=2.0.3.1&extension_version=6.1.0');  
	...
	public function backup_files_by_mbooth($mbooth_xml, $action = 'install'){
		...
		if (!file_exists(DIR_APPLICATION . 'mbooth/backup/')) {
		    mkdir(DIR_APPLICATION . 'mbooth/backup/', 0777, true);
		}

		$mbooth = $this->get_files_by_mbooth(DIR_APPLICATION . 'mbooth/xml/' . $mbooth_xml);
		$files = $mbooth['files'];

		$zip->open(DIR_APPLICATION . 'mbooth/backup/' . date('Y-m-d.h-i-s'). '.'. $action .'.'.$mbooth_xml.'.v'.$mbooth['version'].'.zip', ZipArchive::CREATE);
